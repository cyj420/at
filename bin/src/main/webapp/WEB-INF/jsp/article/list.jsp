...
<body>
	...
	<c:if test="${param.searchKeyword.length() != 0 }">
		<h2>${param.searchKeyword }에 대한 검색 결과</h2>
	</c:if>
		...
		<div>
			<c:forEach var="i" begin="1" end="${fullPage}" step="1">
				<c:if test="${param.page == i }">
					<span>
						<a href="?searchKeyword=${param.searchKeyword}&page=${i}" style="color: red; font-weight: bold;">[${i }]</a>
					</span>
				</c:if>
				<c:if test="${param.page != i }">
					<span>
	...
	<c:if test="${param.searchKeyword.length() != 0 || articles.size() == 0 }">
		<div><a href="/article/list?searchKeyword=&page=1">처음으로 돌아가기</a></div>
	</c:if>
